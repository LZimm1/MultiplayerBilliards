...
public class mouse : MonoBehaviour
{
        ...
        if(gameManagerRef.view.IsMine){
            if(needToPlace){
                ...
                if(placed&&Input.GetButtonDown("Cancel")){
                    needToPlace = true;
                    placed = false;
                    gameManagerRef.view.RPC("chosePocket",RpcTarget.All,new Vector3(100f,0f,0f));
                    pocket=null;
                }
            }
    ...
    void OnTriggerStay2D(Collider2D collider){
        if(collider.CompareTag("pocket1")){
            ...
            gameManagerRef.view.RPC("chosePocket",RpcTarget.All,position);
